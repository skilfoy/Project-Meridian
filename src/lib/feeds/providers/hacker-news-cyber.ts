...
export async function fetchHackerNewsCyber(params: FeedParams): Promise<RawFeedResult> {
  ...
  if (!res.ok) throw new Error(`The Hacker News RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'The Hacker News Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['hacker-news-cyber', 'cyber', 'vulnerability', 'breach', 'apt'],
  }));

  return {
    items,
    meta: { source: 'hacker-news-cyber', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
