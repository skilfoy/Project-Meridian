...
export async function fetchDefenseOne(params: FeedParams): Promise<RawFeedResult> {
  const start = Date.now();
  const limit = Math.min(params.limit ?? 20, 50);

  const res = await fetch('https://www.defenseone.com/rss/all/', {
    next: { revalidate: 1800 },
  });
  if (!res.ok) throw new Error(`Defense One RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'Defense One Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['defense-one', 'government', 'defense', 'military', 'pentagon'],
  }));

  return {
    items,
    meta: { source: 'defense-one', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
