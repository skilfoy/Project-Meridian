...
export async function fetchCisaAdvisories(params: FeedParams): Promise<RawFeedResult> {
  const start = Date.now();
  const limit = Math.min(params.limit ?? 20, 50);

  const res = await fetch('https://www.cisa.gov/sites/default/files/feeds/alerts.xml', {
    next: { revalidate: 3600 },
  });
  if (!res.ok) throw new Error(`CISA Advisories RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'CISA Advisory',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['cisa-advisories', 'cyber', 'ics', 'critical-infrastructure', 'advisory'],
  }));

  return {
    items,
    meta: { source: 'cisa-advisories', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
