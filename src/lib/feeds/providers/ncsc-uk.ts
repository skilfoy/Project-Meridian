...
export async function fetchNcscUk(params: FeedParams): Promise<RawFeedResult> {
  const start = Date.now();
  const limit = Math.min(params.limit ?? 20, 50);

  const res = await fetch('https://www.ncsc.gov.uk/api/1/services/v1/news/rss.xml', {
    next: { revalidate: 3600 },
  });
  if (!res.ok) throw new Error(`NCSC UK RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'NCSC UK Alert',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['ncsc-uk', 'cyber', 'advisory', 'government', 'uk'],
  }));

  return {
    items,
    meta: { source: 'ncsc-uk', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
