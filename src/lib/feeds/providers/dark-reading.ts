...
export async function fetchDarkReading(params: FeedParams): Promise<RawFeedResult> {
  const start = Date.now();
  const limit = Math.min(params.limit ?? 20, 50);

  const res = await fetch('https://www.darkreading.com/rss/all.xml', {
    next: { revalidate: 900 },
  });
  if (!res.ok) throw new Error(`Dark Reading RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'Dark Reading Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['dark-reading', 'cyber', 'threat-intel', 'vulnerability', 'attack'],
  }));

  return {
    items,
    meta: { source: 'dark-reading', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
