...
export async function fetchAlJazeera(params: FeedParams): Promise<RawFeedResult> {
  ...
  if (!res.ok) throw new Error(`Al Jazeera RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'Al Jazeera Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['al-jazeera', 'news', 'world', 'middle-east', 'geopolitics'],
  }));

  return {
    items,
    meta: { source: 'al-jazeera', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
