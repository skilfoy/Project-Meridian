...
export async function fetchBleepingcomputer(params: FeedParams): Promise<RawFeedResult> {
  ...
  if (!res.ok) throw new Error(`BleepingComputer RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'BleepingComputer Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['bleepingcomputer', 'cyber', 'breach', 'ransomware', 'malware'],
  }));

  return {
    items,
    meta: { source: 'bleepingcomputer', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
