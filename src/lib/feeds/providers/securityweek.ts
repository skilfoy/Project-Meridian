...
export async function fetchSecurityweek(params: FeedParams): Promise<RawFeedResult> {
  const start = Date.now();
  const limit = Math.min(params.limit ?? 20, 50);

  const res = await fetch('https://feeds.feedburner.com/securityweek', {
    next: { revalidate: 900 },
  });
  if (!res.ok) throw new Error(`SecurityWeek RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'SecurityWeek Article',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['securityweek', 'cyber', 'vulnerability', 'breach', 'threat-intel'],
  }));

  return {
    items,
    meta: { source: 'securityweek', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
