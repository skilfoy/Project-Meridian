...
export async function fetchBbcWorld(params: FeedParams): Promise<RawFeedResult> {
  ...
  if (!res.ok) throw new Error(`BBC World RSS HTTP ${res.status}`);

  const xml = await res.text();
  const raw = parseRssItems(xml).slice(0, limit);

  const items = raw.map((r) => ({
    title:       r.title || 'BBC World News',
    summary:     r.description?.replace(/<[^>]+>/g, '').slice(0, 300),
    url:         r.link,
    publishedAt: r.pubDate ? new Date(r.pubDate).toISOString() : undefined,
    tags:        ['bbc-world', 'news', 'world', 'geopolitics'],
  }));

  return {
    items,
    meta: { source: 'bbc-world', fetchedAt: new Date().toISOString(), latencyMs: Date.now() - start, total: items.length },
  };
}
